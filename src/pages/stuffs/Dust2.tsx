...
import { dust2Lineups } from "@/data/dust2Lineups";
...
export default function Dust2Stuffs() {
  const cols = 26;
  const rows = 20;
  ...
  const visibleLineups = useMemo(() => {
    if (grenadeFilter === "all") return dust2Lineups;
    return dust2Lineups.filter((l) => l.type === grenadeFilter);
  }, [grenadeFilter]);

  const selectedLineup = useMemo(() => {
    if (!selectedId) return null;
    return dust2Lineups.find((l) => l.lineupId === selectedId) ?? null;
  }, [selectedId]);
