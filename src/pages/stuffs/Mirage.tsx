...
// Adapte si tu as déjà ces types ailleurs
...
export default function MirageStuffs() {
  const cols = 26;
  const rows = 20;
  ...
  // Tooltip + preview errors (⚠️ state global, PAS dans map)
  ...
  // Prevent page scroll (like before)
  useEffect(() => {
  ...
  const visibleLineups = useMemo(() => {
    if (grenadeFilter === "all") return mirageLineups;
    return mirageLineups.filter((l) => l.type === grenadeFilter);
  }, [grenadeFilter]);

  const selectedLineup = useMemo(() => {
    if (!selectedId) return null;
    return mirageLineups.find((l) => l.lineupId === selectedId) ?? null;
  }, [selectedId]);

  // Map size for clamping
