...
import { infernoLineups } from "@/data/infernoLineups";
...
export default function InfernoStuffs() {
  const cols = 26;
  const rows = 20;
  ...
  const visibleLineups = useMemo(() => {
    if (grenadeFilter === "all") return infernoLineups;
    return infernoLineups.filter((l) => l.type === grenadeFilter);
  }, [grenadeFilter]);

  const selectedLineup = useMemo(() => {
    if (!selectedId) return null;
    return infernoLineups.find((l) => l.lineupId === selectedId) ?? null;
  }, [selectedId]);
        ...
        <div className="h-full w-full flex items-center justify-center">
          <div ref={mapRef} className="relative overflow-visible" style={squareSizeStyle}>
            <div className="absolute inset-0 overflow-hidden">
              <img
                src="/maps/inferno.png"
                alt="Inferno overview"
