...
import { nukeLineups } from "@/data/nukeLineups";
...
export default function NukeStuffs() {
  const cols = 26;
  const rows = 20;
  ...
  const visibleLineups = useMemo(() => {
    if (grenadeFilter === "all") return nukeLineups;
    return nukeLineups.filter((l) => l.type === grenadeFilter);
  }, [grenadeFilter]);

  const selectedLineup = useMemo(() => {
    if (!selectedId) return null;
    return nukeLineups.find((l) => l.lineupId === selectedId) ?? null;
  }, [selectedId]);
