...
import { overpassLineups } from "@/data/overpassLineups";
...
export default function OverpassStuffs() {
  const cols = 26;
  const rows = 20;
  ...
  const visibleLineups = useMemo(() => {
    if (grenadeFilter === "all") return overpassLineups;
    return overpassLineups.filter((l) => l.type === grenadeFilter);
  }, [grenadeFilter]);

  const selectedLineup = useMemo(() => {
    if (!selectedId) return null;
    return overpassLineups.find((l) => l.lineupId === selectedId) ?? null;
  }, [selectedId]);
        ...
        <div className="h-full w-full flex items-center justify-center">
          <div ref={mapRef} className="relative overflow-visible" style={squareSizeStyle}>
            <div className="absolute inset-0 overflow-hidden">
              <img
                src="/maps/overpass.png"
                alt="Overpass overview"
