...
import { anubisLineups } from "@/data/anubisLineups";
...
export default function AnubisStuffs() {
  const cols = 26;
  const rows = 20;
  ...
  const visibleLineups = useMemo(() => {
    if (grenadeFilter === "all") return anubisLineups;
    return anubisLineups.filter((l) => l.type === grenadeFilter);
  }, [grenadeFilter]);

  const selectedLineup = useMemo(() => {
    if (!selectedId) return null;
    return anubisLineups.find((l) => l.lineupId === selectedId) ?? null;
  }, [selectedId]);
